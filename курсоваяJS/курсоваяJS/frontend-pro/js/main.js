...
function tippyProps(elem, value) {
    tippy(elem, {
        content: value,
    });
}
        ...
        arrData.forEach(objData => {
            ...
            objData.contacts.forEach((el) => {
                if (el.type) {
                    ...
                    let toolTipValues = el.type + ': ' + el.value;// добавляем тип изначение в TOOLTYPE


                    if (el.type == 'Телефон') {
                        let phonImg = document.createElement('img');
                        phonImg.src = 'img/phoneICON.svg';
                        tippyProps(phonImg, toolTipValues)
